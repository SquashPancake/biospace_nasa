...
function App() {
  ...
  const quickSuggestions = [
    "What is the final conclusion of the paper, Epigenomic regulators elongator complex subunit 2 and methyltransferase 1 differentially condition the spaceflight response in Arabidopsis, regarding genome DNA methylation?",
    "Which two epigenomic regulators were found to differentially condition the spaceflight response in Arabidopsis?", 
    "Based on the Beyond low-Earth orbit article, what is suggested to be the principal immune response generated during deep space exposures?",
    "What is the maximum concentration (wt%) that hydrated magnesium sulfate mineral levels can reach in Martian regolith?"
  ];
  ...
  const processRobotChatInput = async (transcript) => {
    ...
    try {
      ...
      speakText("I encountered an error. Please try speaking again.");
    }
  };
  ...
  const renderMessageActions = (message) => (
    <div className="message-actions">
      <button
        ...
        onClick={() => isSpeaking ? stopSpeaking() : speakText(message.text.replace(/\*\*/g, '').replace(/\*/g, ''))}
