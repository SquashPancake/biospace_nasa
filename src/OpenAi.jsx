...
function App() {
  ...
  const stopAllSpeech = () => {
    ...
    if (openAIAudioRef.current) {
      try {
        ...
        console.log('OpenAI audio stopped');
      } catch (error) {
        console.error('Error stopping OpenAI audio:', error);
      }
    }
  ...
  // WORKING OpenAI TTS Implementation
  const speakWithOpenAITTS = async (text, messageId = null) => {
    ...
    const limitedText = cleanText.substring(0, 2000); // Limit text length
    
    console.log('Starting OpenAI TTS with text:', limitedText.substring(0, 100) + '...');
    
    try {
      // Use the OpenAI SDK directly
      const response = await openai.audio.speech.create({
        model: "tts-1", // Use tts-1 for reliability
        voice: "nova",
        input: limitedText,
        response_format: "mp3"
      });
      ...
      return new Promise((resolve, reject) => {
        ...
        speechTimeoutRef.current = setTimeout(() => {
          console.log('OpenAI TTS safety timeout');
          if (openAIAudioRef.current === audio) {
            audio.pause();
            openAIAudioRef.current = null;
          }
          URL.revokeObjectURL(audioUrl);
  ...
  // Enhanced Text-to-Speech with OpenAI as primary
  const speakText = async (text, messageId = null) => {
    // If already speaking the same message, stop speech completely
    ...
    console.log('Attempting to speak text with OpenAI TTS...');
    
    try {
      await speakWithOpenAITTS(text, messageId);
  ...
  // Process robot chat input with backend integration
  ...
  // Close robot chat and save to history
  ...
  // Render message actions - UPDATED to track which message is speaking
  const renderMessageActions = (message) => {
    ...
    return (
      <div className="message-actions">
        <button
          className={`action-button ${isThisMessageSpeaking ? 'active-speaking' : ''}`}
          onClick={() => isThisMessageSpeaking ? stopAllSpeech() : speakText(message.text, message.id)}
  ...
  const renderRobotChat = () => (
    <div className="robot-chat-overlay">
      <div className="robot-chat-container">
        ...
        <div className="robot-disclaimer">
        ...
        <div className="modal-overlay" onClick={() => setShowInfoModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <h3>TTS Services</h3>
              <ul>
                <li><strong>Primary:</strong> OpenAI TTS (High Quality)</li>
                <li><strong>Fallback:</strong> Browser TTS</li>
              </ul>
            </div>
          </div>
        </div>
